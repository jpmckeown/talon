...
export class CardBackSelectorScene extends Phaser.Scene {
  ...
  public create(): void {
    ...
    this.add.text(this.scale.width / 2.1, 50 * UI_CONFIG.scale, 'Choose Card Back', {
  ...
  #displayCardBackOptions(): void {
    const startX = this.scale.width / 2 - (CARD_WIDTH * 1.5 + 30 * UI_CONFIG.scale);
    const cardY = 180 * UI_CONFIG.scale;
    const spacing = CARD_WIDTH + 40 * UI_CONFIG.scale;

    const options = Object.values(CARD_BACK_OPTIONS);
